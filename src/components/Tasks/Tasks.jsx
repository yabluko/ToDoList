...
function Tasks({ list , onItem , newTask}) {
    const editTitle = () => {
        const newTitle = window.prompt('New title', list.name)
        if (newTitle !== list.name){
            onItem(list.id , newTitle)
            fetch('http://localhost:3001/lists/'+ list.id,{ method: 'PATCH', headers:{'Content-Type': 'application/json' }, body: JSON.stringify({
                name: newTitle
              })}
              ).catch(error => {alert(`Can't update task title ;( ${error}`)})
        }
    }
    ...
    return (
        <div className='tasks'>
            ...
            <div className='tasks__items'>
                {!list.tasks.length  && <h3>No tasks</h3>}
